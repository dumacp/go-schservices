...
func (a *Actor) Receive(ctx actor.Context) {
	...
	case *services.TakeShiftMsg:
		...
		data := takeSvc.Bytes()
		if res, err := ctx.RequestFuture(a.pidData, &gwiotmsg.HttpPostRequest{
			Url:  fmt.Sprintf("%s%s", a.url, constan.URL_SVC_TAKE_SHIFT),
			Data: data,
		}, 10*time.Second).Result(); err != nil {
			ctx.Respond(err)
			break
			...
			if len(resResponse.Error) > 0 {
				...
				if start != -1 {
					...
					ctx.Respond(&services.TakeShiftResponseMsg{Error: details.Msg})
				} else {
					ctx.Respond(&services.TakeShiftResponseMsg{Error: resResponse.Error})
				}
			} else if resResponse.Code == 200 {
	...
	case *services.TakeServiceMsg:
		...
		data := takeSvc.Bytes()
		if res, err := ctx.RequestFuture(a.pidData, &gwiotmsg.HttpPostRequest{
			Url:  fmt.Sprintf("%s%s", a.url, constan.URL_NEW_SVC_COMMAND),
			Data: data,
		}, 10*time.Second).Result(); err != nil {
			ctx.Respond(err)
			break
